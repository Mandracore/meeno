define ([
	...
	return describe("Browser", function() {
		...
		describe("Body", function() {
			...
			it("should create the correct number of note, task and tag subviews", function() {
			...
			xit("should embed the related tags in notes and tasks", function() {
				expect(browserBody.children.notes[0].$el).toContain('span.tags');
				expect(browserBody.children.notes[0].$('span.tags span').eq(0)).toContainText("New test tag");
				expect(browserBody.children.tasks[0].$el).toContain('span.tags');
				expect(browserBody.children.tasks[0].$('span.tags span').eq(0)).toContainText("New test tag");
			});

			describe("when a collection is updated", function() {

				it("should relaunch notes rendering if we add/remove a note or update one (only for title and tagLinks, not for content)", function() {
					var count = 0;
		...
		//#############################################
		// To be put in place
		xdescribe("Body Objects", function() {
			beforeEach(function () {
				loadFixtures('clientSideTemplates.html'); // Inserting the client side templates into the fixtures area of the DOM
				appendLoadFixtures('serverGenerated.html'); // Inserting the HTML generated by Jade on server side into the fixtures area of the DOM
				browserBody = new BrowserBodyView({ el: $("#tabs .browse") }); // Initialize browser
		...
		xdescribe("when a collection is modified", function() {
